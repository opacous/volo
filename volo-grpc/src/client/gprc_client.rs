...
pub trait GrpcClient<T> {
    fn conf_send_compressed(mut self, encoding: CompressionEncoding) -> Self;
    fn conf_send_uncompressed(mut self) -> Self;

    fn conf_max_encoding_msg_size(mut self, limit: usize) -> Self;

    async fn send_unary_req<M1, M2, C>(
        &mut self,
        request: Request<M1>,
        path: PathAndQuery,
        codec: C,
    ...
    async fn client_streaming<S, M1, M2, C>(
        &mut self,
        request: Request<S>,
        path: PathAndQuery,
        encoder: M1,
